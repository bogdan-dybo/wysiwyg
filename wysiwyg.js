...
(function (root, factory) {
  ...
  class WysiwygEditor {
    ...
    _bindEvents() {
      ...
      // Paste: strip word/browser garbage, keep plain structure
      editor.addEventListener('paste', (e) => {
        e.preventDefault();
        const html = e.clipboardData.getData('text/html');
        const text = e.clipboardData.getData('text/plain');
        // Use HTML if available (lets formatted content through)
    ...
    _onKeydown(e) {
      ...
      if (e.key === 'Tab') {
        ...
        if (cell) {
          ...
          // Indent with 4 non-breaking spaces
    ...
    _updateState() {
      ...
      // Heading dropdown item highlight
     ...
     * @param {string}   event  'change' | 'focus' | 'blur'
     * @param {Function} fn
     * @returns {WysiwygEditor} this
     */
    on(event, fn) {
    ...
    _toggleSource() {
      ...
      if (this._isSourceMode) {
        ...
        this._toolbar.querySelectorAll('.wysiwyg-heading-btn, .wysiwyg-color-btn').forEach(b => b.disabled = true);
      } else {
        // Switch back to visual editor — apply source edits
    ...
    /**
     * Removes the editor and restores the original element.
     */
    destroy() {
